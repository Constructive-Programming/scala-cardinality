...
class SizeSpec extends Specification {
  def is = s2"""
    TinySize equality compares cardinality     ${ (TinySize(4) !== TinySize(2)) and (TinySize(4) === TinySize(4)) }
    ...
    Nothing absorbs products                   ${ (NothingSize * IntSize === NothingSize) and (EffectiveOmega * NothingSize === NothingSize) }
    Omega times Tau is Tau                     ${ EffectiveOmega * EffectiveTau === EffectiveTau }
    Omega is larger than finite                ${ EffectiveOmega.larger(IntSize) and !IntSize.larger(EffectiveOmega) }
    Tau is larger than Omega                   ${ EffectiveTau.larger(EffectiveOmega) and !EffectiveOmega.larger(EffectiveTau) }
    larger is antisymmetric for lossy sizes    ${ FloatSize.larger(LongSize) and !LongSize.larger(FloatSize) }
    tiny to a finite power terminates          ${ (BooleanSize ^ IntSize) === FiniteSize(BigInt(1) << 32) }
    ...
    huge finite exponents become Omega         ${ (IntSize ^ FiniteSize(BigInt(Int.MaxValue) + 1)) === EffectiveOmega }
